...
async function loadCookDashboard() {
  try {
    ...
    // total served
    document.getElementById("menus-served-val").textContent =
      data.total_served || 0;
    ...
    if (avgRatingEl)
      avgRatingEl.textContent = data.avg_rating ? data.avg_rating + " ★" : "-";

    // avg cook time
    const avgTimeEl = document.getElementById("avg-time-val");
    if (avgTimeEl)
      avgTimeEl.textContent =
        data.avg_cook_time !== null && data.avg_cook_time !== undefined
...
function renderTopMenus() {
  ...
  document.getElementById("topMenus").innerHTML = topMenus
    .map(
      (menu, i) => `
...
function renderHourlyChart() {
  ...
  document.getElementById("hourlyChart").innerHTML = slots
    .map(
      (h) => `
        <div class="flex items-center gap-3">
