...
app.post('/cook/login', async function (req, res) {
    ...
    const sql = "SELECT cook_id, username, password, status FROM cooks WHERE username = ?";
...
app.post('/cook/register', function (req, res) {
    ...
    function insertWithUsername(username) {
        ...
        const insertSql = "INSERT INTO cooks (username, password, status) VALUES (?, ?, 'disable')";
        con.query(insertSql, [username, hash], function (err, result) {
            if (err) {
                console.error('Insert cook error', err);
                return res.status(500).send('Server error');
            }
            return res.status(201).json({ message: 'Account created', cookId: username });
        });
    }
    ...
    if (cookId && String(cookId).trim()) {
        ...
        const maxSql = "SELECT MAX(CAST(SUBSTRING(username,2) AS UNSIGNED)) AS maxnum FROM cooks WHERE username REGEXP '^C[0-9]+'";
...
// Admin helper: drop status_note column from cooks if it exists
app.post('/admin/db/drop-status-note', function (req, res) {
    con.query("SHOW COLUMNS FROM cooks LIKE 'status_note'", function (err, cols) {
        if (err) {
            console.error('Column check error', err);
            return res.status(500).json({ message: 'DB error' });
        }
        if (!Array.isArray(cols) || cols.length === 0) {
            return res.status(200).json({ message: 'status_note column not found' });
        }

        con.query('ALTER TABLE cooks DROP COLUMN status_note', function (derr) {
            if (derr) {
                console.error('Drop column failed', derr);
                return res.status(500).json({ message: 'Failed to drop column', error: derr.sqlMessage || derr.message });
            }
            res.status(200).json({ message: 'status_note column dropped' });
        });
    });
});
...
// recent served items for cook dashboard
app.get('/cook/recent-served', function (req, res) {
    ...
    con.query(sql, function (err, results) {
        if (err) {
            ...
            return res.status(500).json({ message: 'Unable to fetch recent served items' });
        }
        res.json(results);
    });
});
